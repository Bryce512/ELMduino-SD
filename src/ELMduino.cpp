...
double ELM327::processPID(const uint8_t &service, const uint16_t &pid, const uint8_t &num_responses, const uint8_t &numExpectedBytes, const double &scaleFactor, const float &bias)
{
    ...
    //     Serial.println("Command Sent. Waiting Response");
    //     nb_query_state = WAITING_RESP;
    // }
    // else if (nb_query_state == WAITING_RESP)
    // {
    //     Serial.println("Running get_response()");
    //     get_response();
    //     if (nb_rx_state == ELM_SUCCESS)
    //     {
    //         Serial.println("Reponse Successful!");
    ...
    // return 0.0;
    // Serial.println("Entered ProcessPID()");
    while(true)  {
        // Serial.print("nb_query_state: "); Serial.println(nb_query_state);
        // Serial.print("nb_rx_state: "); Serial.println(nb_rx_state);
        switch (nb_query_state){
            ...
            case WAITING_RESP:
                // Serial.println("Running get_response()");
                    ...
                    {
                        Serial.println("Response Timeout. Resetting query state.");
                        nb_query_state = SEND_COMMAND; // Timeout occurred, reset the query state machine for next command
                        return 0.0;
                    }
                    else if (nb_rx_state != ELM_GETTING_MSG)
                    {
                        Serial.println("Error receiving response. Resetting query state.");
                        nb_query_state = SEND_COMMAND; // Error occurred, reset the query state machine for next command
                        return 0.0;
                    }
                ...
                break;
            default:
                Serial.println("Error: Invalid query state.");
                return 0.0;
        }
    }
...
uint32_t ELM327::supportedPIDs_1_20()
{
    Serial.println("Begin ProcessPID() for PID ");
...
/*
 ...
 Return:
 -------
  * float - Engine Coolant Temp in C
*/
float ELM327::engineCoolantTemp()
{
    tempC = processPID(SERVICE_01, ENGINE_COOLANT_TEMP, 1, 1, 1, -40.0);
    // return (tempC*(9/5) + 32);
    return tempC;
}
...
/*
 uint32_t ELM327::supportedPIDs_81_A0()

 Description:
 ------------
  * Determine which of PIDs 0x81 through 0xA0 are supported (bit encoded)
 ...
 Return:
 -------
  * uint32_t - Bit encoded booleans of supported PIDs 0x81-0xA0
*/
uint32_t ELM327::supportedPIDs_81_A0()
{
    return (uint32_t)processPID(SERVICE_01, SUPPORTED_PIDS_81_A0, 1, 4);
}

/*
 uint32_t ELM327::supportedPIDs_A1_C0()

 Description:
 ------------
  * Determine which of PIDs 0xA1 through 0xC0 are supported (bit encoded)
 ...
 Return:
 -------
  * uint32_t - Bit encoded booleans of supported PIDs 0xA1-0xC0
*/
uint32_t ELM327::supportedPIDs_A1_C0()
{
    return (uint32_t)processPID(SERVICE_01, SUPPORTED_PIDS_A1_C0, 1, 4);
}

/*
 uint32_t ELM327::supportedPIDs_C1_E0)

 Description:
 ------------
  * Determine which of PIDs 0xC1 through 0xE0 are supported (bit encoded)
 ...
 Return:
 -------
  * uint32_t - Bit encoded booleans of supported PIDs 0xC1-0xE0
*/
uint32_t ELM327::supportedPIDs_C1_E0()
{
    return (uint32_t)processPID(SERVICE_01, SUPPORTED_PIDS_C1_E0, 1, 4);
}
...
/*
 ...
 Return:
 -------
  * uint16_t - total vehicle mileage in km
*/
uint16_t ELM327::odometerReading()
{
    return (uint16_t)processPID(SERVICE_01,ODOMETER_MILEAGE,1,4);
}
...
bool ELM327::isPidSupported(uint8_t pid)
{
    ...
    // Calculate the byte index and bit position for the adjusted PID
        ...
        Serial.print("Byte Index number: "); Serial.println(byteIndex);
        Serial.print("Bit position: "); Serial.println(bitPosition);
        Serial.print("Bit mask: "); Serial.println(bitMask);
        Serial.print("Index Value: "); Serial.println(byteIndex);

        // Check if the corresponding bit is set in the response
        bool supported = (indexByte & bitMask) != 0;
        Serial.print("PID supported?--> "); Serial.println(supported);
...
/*
  ...
  * bool - Whether or not the queried PID is supported by the ECU.
*/
uint32_t ELM327::getPIDInterval(uint8_t pid) {
//    Serial.print("PID passed into getPIDInterval(): "); Serial.println(pid);
   if (pid >= 0x00 and pid <= 0x20) {
        pidInterval = SUPPORTED_PIDS_1_20;
    } else if (pid >= 0x21 and pid <= 0x40) {
        pidInterval = SUPPORTED_PIDS_21_40;
    } else if (pid >= 0x41 and pid <= 0x60) {
        pidInterval = SUPPORTED_PIDS_41_60;
    } else if (pid >= 0x61 and pid <= 0x80) {
        pidInterval = SUPPORTED_PIDS_61_80;
    } else if (pid >= 0x81 and pid <= 0xA0) {
        pidInterval = SUPPORTED_PIDS_81_A0;
    } else if (pid >= 0xA1 and pid <= 0xC0) {
...
/*
 uint8_t ELM327::processPIDInterval(uint32_t pidInterval, uint8_t pid)

 Description:
 ------------
  * Takes the pidInterval and runs the corresponding query to the ECU. 
  * The response from the vehicle is a 4 byte bit-encoded response that needs
  * to be process in order to know which of the 32 pids are supported.
  
  * The 4 bytes (32 bits) represent the 32 pids in and whether or not they are supported.
           - see "Bitwise encoded PIDs" on the Wikipedia https://en.wikipedia.org/wiki/OBD-II_PIDs

 Inputs:
 -------
  * uint32_t pidInterval - the corresponding PID interval that contains the 
        * PID we are requesting. Each interval contains 32 PIDs
        * but are written as 1-20 because that is the Hexidecimal code. 0x01 = 1 while 0x20 = 32
  * uint8_t pid - the ORIGINAL PID to check for support.

 Return:
 -------
  * uint8_t pid - returns the ADJUSTED pid after taking into account the interval
*/
uint8_t ELM327::processPIDInterval(uint8_t pid) {
    pidInterval = getPIDInterval(pid);
    switch (pidInterval)
        {
        ...
        case SUPPORTED_PIDS_61_80:
            ...
            pid -= 0x60;
            break;
        case SUPPORTED_PIDS_81_A0:
            supportedPIDs_81_A0();
            pid -= 0x20;
            break;

        case SUPPORTED_PIDS_A1_C0:
            supportedPIDs_A1_C0();
            pid -= 0xA0;
            break;
        
        case SUPPORTED_PIDS_C1_E0:
            supportedPIDs_C1_E0();
            pid -= 0xC0;
            break;
    ...
    Serial.print("Adjusted PID to be indexed: ");Serial.println(pid);
    return pid;
}

/*
 bool ELM327::getResponseByte(byteIndex)

 Description:
 ------------
  * Returns the byte with in the response for a given index value when decoding bit encoded responses.

  * This is a helper method that selects the correct byte from an ECU response 
  * and returns the byte to be used in the isPIDsupported() method. 
  * Within this isPIDsupported method, the byte is decoded and the bool value can be determined.

 Inputs:
 -------
  * uint8_t byteIndex - the Index of the byte to be decoded as calculated in the isPIDsupported() method.

 Return:
 -------
  * byte - the byte value that contains the encoded message. 
*/
byte ELM327::getResponseByte(uint8_t byteIndex) {
    switch (byteIndex)
    {
    case 0:
        return responseByte_0;
        break;
    case 1:
        return responseByte_1;
        break;
    case 2:
        return responseByte_2;
        break;
    case 3:
        return responseByte_3;
        break;
    case 4:
        return responseByte_4;
        break;
    case 5:
        return responseByte_5;
        break;
    case 6:
        return responseByte_6;
        break;
    case 7:
        return responseByte_7;
        break;
