...
const uint8_t SUPPORTED_PIDS_81_A0     				 = 128; // 0x80 - bit encoded

const uint8_t SUPPORTED_PIDS_A1_C0     				 = 160; // 0xA1 - bit encoded
const uint8_t ODOMETER_MILEAGE								 = 166; // 0xA6 - km

const uint8_t SUPPORTED_PIDS_C1_E0     				 = 192; // 0xC1 - bit encoded
...
// Declare the pidToFunctionNameMap as an extern variable
extern std::unordered_map<uint16_t, std::pair<std::function<double()>, std::string>> pidToFunctionNameMap;
...
public:
	...
	bool processEncodedIndex(uint8_t pid);
	...
	uint32_t getPIDInterval(uint8_t pid);
	uint8_t processPIDInterval(uint8_t pid);
	byte getResponseByte(uint8_t byteIndex);
	...
	uint32_t supportedPIDs_81_A0();
	
	uint32_t supportedPIDs_A1_C0();
	uint16_t odometerReading();
